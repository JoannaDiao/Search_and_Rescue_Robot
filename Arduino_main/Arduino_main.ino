...
void updateCurrentTile(const std::pair<double, double>& position, const int next_tile, int& current_tile) {
  ...
  if(shouldAdjust() && front_tof.getDistance() > 1000) {
    return;  
  }
  ...
  std::pair<int, int> next_coord = coords[path[next_tile].first][path[next_tile].second];
  double next_center_x = next_coord.first;
  double next_center_y = next_coord.second;
...
void handleInit() {
  ...
  adjust_setpoint = getDesLeftDist();
...
bool shouldAdjust() {
  ...
  // set state to adjust state depending on distance from 
  if(adjsut_output > 10 && adjust_input > adjust_setpoint) {
    setState(LEFT_ADJUST);
    return true;
  } else if (adjust_output > 10 && adjust_input < adjust_setpoint) {
...
void handleAdjust(char direction) {
...
void setup() {
  ...
  adjustController.begin(&adjust_input, &adjust_output, &adjust_setpoint, adujst_p, adjust_i, adjust_d);
...
void loop() {
  ...
  adjust_input = (double)left_tof_distance;
...
/*
TO DO (FILE WITH PID CONTROLLER)
1. implement the library 
2. find adjust_p, adjust_i, adjust_d values
2. set adjust_p, adjust_i, adjust_d values
3. set an initial setpoint value
4. normalized output value
5. change output condition in shouldAdjust() function to be within desired range
*/